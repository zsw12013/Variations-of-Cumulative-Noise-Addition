...
;;schema-features is the data set(waruni)
...
(defn make-random-projection-with-noise-and-translation-mask-factory
  ...
  [projection-features projection-sigma independent-sigma cumulative-sigma translation cycle-size
  ...
  (fn [schema-features]
    (let [seed (or seed 1)
          ...
          projection-mask-factory (make-random-projection-mask-factory
                                   ...
                                   :seed (next-derived-seed! rng))
          mask-factory (decorate-mask-factory  projection-mask-factory identity
                           (fn [features]
                             (let [independent-noise (repeatedly projection-features  #(next-gauss! rng 0 independent-sigma))
                                   cumulative-noise (repeatedly projection-features   #(next-gauss! rng 0 cumulative-sigma))
								                  ;; cumulative-noise-vec (vec cumulative-noise)			
											            ]
                            ...
                            ;; to the features.
                               ...
                               (vec (map + features translation-vector independent-noise  @cumulative-noise-vector-atom))
                            )
                          )
                        )
          ]
...
(defn make-random-projection-with-noise-and-translation-mask-factory-cycles 
  [projection-features projection-sigma independent-sigma cumulative-sigma translation cycle-size growth-rate-k maximum-fn-value
   & {:keys [seed noise-log-atom]}]
  (when noise-log-atom
    (reset! noise-log-atom []))
          ...
          mask-factory (decorate-mask-factory
                        ...
                        (fn [features]
                           (when features (swap! logistic-counter inc)   
                                  ...
                                  (if (> @logistic-counter (* 1 cycle-size))   ;;when logistic counter get the value of cycle cycle reset it to - cycle size (to set the range of x in logistic function)
                                    (reset! logistic-counter   (- (* -1 cycle-size) 1 ) )   ))
                            (let [
                                  logistic-sigma (*   (/ maximum-fn-value  (+ 1    (m/exp (* (* -1 growth-rate-k)  @logistic-counter ) )  )  ) cumulative-sigma )
                                  cumulative-noise   (if (= maximum-fn-value 0)
                                                      ;;(repeatedly projection-features #(m/abs (next-gauss! rng 0 cumulative-sigma)))  ;;absolute values are taking to both adddition and subtraction 
																			;;  (repeatedly projection-features #(m/abs (next-gauss! rng 0 logistic-sigma)))
																			(repeatedly projection-features #(next-gauss! rng 0 cumulative-sigma))
																			(repeatedly projection-features #(next-gauss! rng 0 logistic-sigma))
																			                                                    )	
									         	     	]
                                  ;;Creating cycles..................   
                             (when features (swap! counter inc) )
                                  (if (> @counter (* 2 cycle-size))
                                       (do
                                           (reset! counter 1 )  
								                                   ;;can check this with both with/without resetting the noise level.
								                                   ;;Reset noise --> low privacy and high accuracy.  without resetting --> High noise/privacy and low accuracy
                                            ;; (reset! cumulative-noise-vector-atom (m/zero-vector (count schema-features))) 
                                    )
                                 )
																	                                ;;    (cond 
																	                                ;; (<= @counter (* 1 cycle-size))
																	                                ;;  (swap! cumulative-noise-vector-atom
																	                                ;;   #(map + % cumulative-noise))
																	                                ;;     :else            ;;waruni(and (<= @counter (* cycle-size 2)) (> @counter (* 1 cycle-size )))
																	                                ;;     (swap! cumulative-noise-vector-atom
																	                                ;;   #(map - % cumulative-noise))  
																	                                      ;;wauni:else (let  [cumulative-noise-vector-atom (atom (m/zero-vector (count schema-features)))])
																	                                ;;     ) 
                                       (swap! cumulative-noise-vector-atom
																	                  #(map + % cumulative-noise))
                                    ;;   (save-data-append-newline (str "noise2.csv")  cumulative-noise)
                                     ;;  (println "cumulative-noise" cumulative-noise)
                                      ;; (println "vector atom" @cumulative-noise-vector-atom)
