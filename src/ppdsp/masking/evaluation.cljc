  ...
  (:require [ppdsp.masking.base :refer [mask-dataset
            ...
            [ppdsp.masking.projection
             :refer [make-random-projection-with-noise-and-translation-mask-factory
                     make-random-projection-with-noise-and-translation-mask-factory-cycles]]        
...
;;waruni
;;(defn test-classification-accuracy-cycles
;;  [classifier-fn dataset cycle-size]
;;  (let [results (train-classifier (classifier-fn (get-schema dataset)) dataset)]
  ;;  {:accuracy (get-accuracy-cycles results cycle-size)
   ;;  :raw-results results})
;; )
...
(defn get-record-bounds
  ...
  [flat-record-length record-range range-position]
  (if (= 1 record-range)
    ;; Short-circuit for full range.
    [0 flat-record-length]
    (case range-position
      :middle (let [onesided-bound (Math/round (* flat-record-length (* record-range 0.5)))
                    mid-record (quot flat-record-length 2)]
                ;; Return the start and end bounds either side of the middle.
                [(max 0 (- mid-record onesided-bound))
                 (min flat-record-length (+ mid-record onesided-bound))])
      :start [0 (Math/round (* flat-record-length record-range))])))
...
(defn generate-io-attack-data!
  ...
  [rng input-matrix masked-matrix projection-sigma independent-sigma cumulative-sigma known-record-count 
   known-record-range known-record-range-position cycle-size flat-record-length cycle-no] ;; 
 
  (def full-cycle (* cycle-size 2))
   (let [  ;;select a cycle randomly according to the number of cycles in the data set (depends on the CS and record count) 
        ;; cycle-no (rand-int 36)
         cycle-no cycle-no
           ;; Length of start flat period and end flat period of the selected cycle   flat-record-length                                     
          [known-record-start known-record-end] (get-record-bounds flat-record-length known-record-range known-record-range-position)    ;;gives [0 record-count]
         ;; Create a derived rng so that the number of known records does not effect subsequent rng in the experiment.
          ...
          unknown-record-index (first (random-li-row-indexes! records-rng input-matrix 1 
                                        ;;for starting flat area 
                                                           ;;   :start-index (+ known-record-start (* full-cycle (- cycle-no 1))) 
                                                             ;; :end-index (+ known-record-end (* full-cycle (- cycle-no 1)) )
                                          ;;For ending flat area
                                                               :start-index (- (* full-cycle cycle-no) known-record-end)
                                                               :end-index ( - (* full-cycle cycle-no) known-record-start )
                                       )
                                 )
          known-record-indexes (random-li-row-indexes! records-rng input-matrix
                                                       known-record-count
                                                       :start-index  known-record-start 
                                                       :end-index (m/row-count masked-matrix )  ;;for the full record range
                                                       :init-li-row-indexes [unknown-record-index]
                                )
         ]
   ;;  (println "records-rng" records-rng)
    ;; (println "cycle no" cycle-no)
  ;;   (println "start" (+ known-record-start (* full-cycle (- cycle-no 1))))
   ;;  (println "end" (+ known-record-end (* full-cycle (- cycle-no 1)) ))
   ...
   (pmap-pool (or evaluation-threads 1)
    (fn [[known-record-count known-record-range rng]]
		      (let [known-record-range-position (or known-record-range-position
		       ...
		       ;; Record contains raw results for this prior-knowledge-configuration.
		        ...
		        :attack-results
				        (doall
					         (for [i (range (or attack-count 100))]
							           ...
							           (let [    ;; We may need to try generate-io-attack-data multiple times, in case the first few selected records end up being linearly dependent with all other records.
				                       cycle-no (if (<= @x (- cycles 1)) ;;send the cycles no to perform attacks for every cycle
				                                  (swap! x inc)
				                                  (reset! x 1)
				                               )
							                   ...
							                   io-attack-data (try-times
							                                   100
							                                    (generate-io-attack-data! (seeded-rng (next-derived-seed! rng))
							                                                              ...
							                                                              cycle-size flat-record-length cycle-no ) ;;waruni 
							                                   )
    ...
    (for [known-record-count (or known-record-counts
          ...
          known-record-range (or known-record-ranges
                                 [0.1 0.5 1])]
      ;; Each configuration needs its own rng to prevent race conditions when multi-threading.
...
(defn masking-experiment
  [& {:keys [raw-dataset projection-features projection-sigma
             ...
             translation  cycle-size classifier-fns privacy-evaluation-configuration
             seed evaluations]}]
...
(defn masking-experiment-cycles
  [& {:keys [raw-dataset projection-features projection-sigma
             independent-noise-sigma cumulative-noise-sigma growth-rate-k maximum-fn-value 
             translation  cycle-size classifier-fns privacy-evaluation-configuration 
             seed evaluations]}]
  (let [rng (seeded-rng seed)
        ;; We max/min normalise the dataset so that the effect of the same sigma (particularly for additive noise) is consistent
        ;; across different datasets. This was previously done in Chen, K., Sun, G., & Liu, L. (2007, April). Towards
        ;; attack-resilient geometric data perturbation. In proceedings of the 2007 SIAM international conference on
        ;; Data mining (pp. 78-89). Society for Industrial and Applied Mathematics.
        dataset (normalise-dataset raw-dataset)
        noise-log-atom (atom [])
        dataset-mask-factory (make-random-projection-with-noise-and-translation-mask-factory-cycles
                              projection-features projection-sigma
                              independent-noise-sigma cumulative-noise-sigma
                              translation cycle-size growth-rate-k maximum-fn-value      ;;waruni
                              :seed (next-derived-seed! rng)
                              :noise-log-atom noise-log-atom)
        masked-dataset (mask-dataset (make-dataset-mask dataset-mask-factory dataset) dataset)
        input-matrix (dataset->matrix dataset)
        ...
        output (zipmap
                ...
                (for [evaluation evaluations]
                  (case evaluation
                    :accuracy (map-vals #(test-classification-accuracy % masked-dataset)    ;;waruni
                                        classifier-fns )
                    :privacy (evaluate-privacy rng input-matrix masked-matrix
                                               projection-sigma independent-noise-sigma
                                               cumulative-noise-sigma translation
